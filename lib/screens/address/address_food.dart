...
class _AddressFoodScreenState extends State<AddressFoodScreen> {
  ...
  Future getPosition() async {
    ...
    print("Services is ${cl}");
    if(cl == LocationPermission.denied ){
      Geolocator.requestPermission();
    }
    cu = await Geolocator.getCurrentPosition();
    print(cu);
    placemarks = await placemarkFromCoordinates(cu!.latitude, cu!.longitude);
    setState(() {
  ...
  void initState() {
    ...
    getPosition();
    if(AddressAppCubit.get(context).placeMark.name!.isEmpty){
      newUser = true;
    } else {
      newUser = false;
    }
                            ...
                            children: [
                              GoogleMap(
                                initialCameraPosition: CameraPosition(
                                  target: !newUser?initialPosition:LatLng(cu!.latitude, cu!.longitude),
                                  zoom: 17,
                                ),
                                onTap: (argument) {
